      ...
      <section className="relative mx-auto max-w-6xl px-5 py-8 md:px-8 md:py-12">
        ...
        <header className="grid gap-10 py-16 lg:grid-cols-[1.1fr_.9fr]">
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}>
            ...
            <h1 className="mt-9 max-w-3xl text-4xl font-semibold leading-tight text-signal-text md:text-6xl">
              Ecosystem memory,
              <br />
              signal epochs,
              <br />
              <span className="text-signal-muted">and daily intelligence briefings.</span>
            </h1>
          </motion.div>
    ...
    <div className="console-panel p-5">
      ...
      <div className="mt-5 space-y-3 text-sm leading-relaxed text-signal-muted">
        {(timeline?.briefing.lines ?? ["Awaiting sufficient real ecosystem activity to form a daily briefing."]).map((line) => (
          <p key={line} className="border-l border-signal-green/40 bg-signal-panel/62 px-3 py-2">{line}</p>
        ))}
      </div>
    </div>
  );
        ...
        {epochs.length === 0 ? (
          ...
          epochs.map((epoch, index) => (
            <motion.div
              ...
              className="border border-signal-line bg-signal-panel/72 p-4"
            >
